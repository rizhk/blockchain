...
class SellOfferApi {
		...
		return new Promise((resolve, reject) => {
			...
			fetch(PicanteApi.SellOffer, {
				...
				.then(
					(data) => {
						if (!data.error) {
							const wallets = <SellOffer[]>data.items;
	...
	getItem(walletId: string): Promise<SellOffer> {
		return new Promise((resolve, reject) => {
			const accessToken =
				globalThis.localStorage.getItem("accessToken") || "";

			fetch(PicanteApi.SellOffer + "/" + walletId, {
				method: "GET",
				headers: {
					"Content-Type": "application/json",
					Authentication: accessToken,
				...
				.then(
					(data) => {
						if (!data.error) {
							const wallets = <SellOffer>data.item;
