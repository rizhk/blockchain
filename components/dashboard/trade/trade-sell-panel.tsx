...
import { useSendTokenModal, useGrantPermissionModal, useTokenTransferedModal } from 'hooks/use-sell-modal';
...
export const SellPanel: FC = (props) => {
  ...
  const { isWalletConnectShowing, toggleWalletConnect } = useWalletConnectModal();

  const { isGrantPermissionShowing, toggleGrantPermission } = useGrantPermissionModal();
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, helpers): Promise<void> => {
      try {
        ...
        console.log(offerId);
        const myTokenContract = new ethers.Contract(tokenAddr, abi, signer);
        ...
        var transfer = await myTokenContract.transfer(DexContractAddr, amountToSellInWei);
